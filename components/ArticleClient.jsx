  ...
  return (
    <>
      ...
      <div style={{ maxWidth:740, margin:'0 auto', padding:'40px 24px' }}>
        ...
        <h1 style={{ fontSize:36, fontFamily:'Barlow Condensed,sans-serif', fontWeight:800, letterSpacing:.5, lineHeight:1.1, color:'var(--text)', marginBottom:16 }}>{article.title.toUpperCase()}</h1>
        ...
        <p style={{ fontSize:16, color:'var(--text2)', lineHeight:1.8, marginBottom:36, borderLeft:'3px solid var(--amber)', paddingLeft:16 }}>{article.summary}</p>
        <div style={{ borderTop:'1px solid var(--border)', paddingTop:32 }}>
          {article.content.map((block, i) => {
            if (block.type==='lead') return <p key={i} style={{ fontSize:16, lineHeight:1.9, color:'var(--text)', marginBottom:28, fontWeight:500 }}>{block.text}</p>;
            if (block.type==='h2')  return <h2 key={i} style={{ fontSize:22, fontFamily:'Barlow Condensed,sans-serif', fontWeight:700, letterSpacing:.5, color:'var(--text)', marginBottom:14, marginTop:36 }}>{block.text.toUpperCase()}</h2>;
            if (block.type==='p')   return <p key={i} style={{ fontSize:15, lineHeight:1.9, color:'var(--text2)', marginBottom:20 }}>{block.text}</p>;
            return null;
          })}
        </div>
