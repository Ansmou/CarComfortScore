    ...
    <>
      ...
      <nav style={{ background:'var(--nav-bg)', borderBottom:'1px solid var(--border)', padding:'0 24px', position:'sticky', top:0, zIndex:100 }}>
        <div style={{ maxWidth:1200, margin:'0 auto', display:'flex', alignItems:'center', height:52, justifyContent:'space-between' }}>
          ...
          <div style={{ display:'flex', gap:2 }}>{NAV.map(([href,l])=><Link key={href} href={href} style={{ padding:'8px 12px', fontSize:10, fontWeight:600, fontFamily:'IBM Plex Mono,monospace', letterSpacing:1, color:href==='/science'?'var(--amber)':'var(--text3)', textDecoration:'none', borderBottom:href==='/science'?'2px solid var(--amber)':'2px solid transparent' }}>{l}</Link>)}</div>
        </div>
      </nav>
      <div style={{ maxWidth:740, margin:'0 auto', padding:'48px 24px' }}>
        ...
        <h1 style={{ fontSize:40, fontFamily:'Barlow Condensed,sans-serif', fontWeight:800, letterSpacing:1, marginBottom:16, lineHeight:1.1, color:'var(--text)' }}>HOW WE MEASURE RIDE COMFORT</h1>
        ...
        {[
          ...
          {code:'04',label:'WBV Impact Penalty',weight:'−penalty',color:'var(--red)',desc:'Corrects for impact character. Low-frequency (4–8 Hz) high-amplitude impacts correlate with lumbar loading per WHO guidelines. Penalty: −0 (multilink/independent), −4 (torsion beam), −10 (coil solid axle), −15 (leaf spring).'},
        ].map(item=>(
          <div key={item.code} style={{ display:'grid', gridTemplateColumns:'48px 1fr', gap:16, marginBottom:28, paddingTop:20, borderTop:'1px solid var(--border)' }}>
            ...
            <div>
              <div style={{ display:'flex', gap:10, alignItems:'center', marginBottom:8, flexWrap:'wrap' }}>
                <h3 style={{ fontSize:16, fontFamily:'Barlow Condensed,sans-serif', fontWeight:700, letterSpacing:.5, color:'var(--text)' }}>{item.label.toUpperCase()}</h3>
