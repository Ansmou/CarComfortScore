...
export function computeImpactChain(car) {
  ...
  // Cap absorption to what's actually left so the displayed math always reconciles:
  // residual_new === residual_prev − absorbed (or + added). No hidden floors.
  let r = 100;
  const stages = IMPACT_CHAIN.map(s => {
    let pts = s.getPoints(car);
    if (pts > 0) pts = Math.min(pts, r);
    r = r - pts;
