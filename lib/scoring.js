...
// Even the best engineering can't absorb 100% of road energy. Floor at 2 —
// low enough that top-tier cars differentiate, high enough to stay realistic.
const FELT_FLOOR = 2;

export function computeImpactChain(car) {
  // Each stage may absorb (pts > 0) or add back (pts < 0) energy.
  // Per-stage cap: a stage can never absorb past the floor — so once residual
  // hits 6, later stages absorb 0 (energy was eaten by upstream components).
  // Display always reconciles: residual_new === residual_prev − absorbed.
  let r = 100;
  const stages = IMPACT_CHAIN.map(s => {
    let pts = s.getPoints(car);
    if (pts > 0) pts = Math.min(pts, Math.max(0, r - FELT_FLOOR));
    r = r - pts;
